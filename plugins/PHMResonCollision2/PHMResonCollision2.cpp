#include "PHMResonCollision2.hpp"
#include "../utils.hpp"
...
PHMResonCollision2::PHMResonCollision2() {
    std::memset(_y1,0.f,sizeof(_y1));
    std::memset(_y2,0.f,sizeof(_y2));
    _cdispl = 0.f;
    _fmin = 0.f;
    _fmax = 0.f;
    mCalcFunc = make_calc_function<PHMResonCollision2, &PHMResonCollision2::next>();
    next(1);
}

void PHMResonCollision2::computeRandomModes( const float fmin,
                                            const float fmax,
                                            const float d1,
                                            const float d2,
                                            const float posin, 
    ...
    for (int i = 0; i < _nmodes; ++i) {
        ...
        // freqs above average are detuned up
        // freqs below average are detuned down     
        float vf = _fc[i];
        vf = sc_clip(_fc[i] * ( 1 + sign(vf-fmean) * spread ), 20.f, flim);
        //float vf = sc_clip(_fc[i] * ( 1 + detune ), 20.f, flim );
        float omega = vf * twopi;
        float d = d1 + d2 * (vf * vf * sr1_squared);
...
void PHMResonCollision2::next(int nSamples) {
    ...
    const float spread = sc_clip(in0(12), 0.f, 1.f);

    computeRandomModes(fmin, fmax, d1, d2, posin, cposin, nmodes_req, detune, spread);
...
PluginLoad(PHMResonCollision2UGens) {
    // Plugin magic
    ft = inTable;
    registerUnit<PHMResonCollision2::PHMResonCollision2>(ft, "PHMResonCollision2", false);
}
