...
PHMResonFriction::PHMResonFriction() {
    ...
    mCalcFunc = make_calc_function<PHMResonFriction, &PHMResonFriction::next>();
    next(1);
}



void PHMResonFriction::next(int nSamples) {
...
void PHMResonFriction::UpdateFilterCoeffs(const float fmin,const float fmax){
    
    float sr = sampleRate();
    float nyq = sr * 0.5f;
    const float sr1 = 1.f / sr;
    const float sr1_squared = sr1 * sr1;
    const float flim = sr * 0.33f;
    float dt = 1.f / sr;
    _nmodes=0;
    ...
    if (fmin != _fmin || fmax != _fmax) {
        ...
        for (int i = 0; i < _nmodes_req; ++i) {
            float rnd = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
            _fc[i] = _fmin + rnd * (_fmax - _fmin);
        }
    }
    float dia = 0.002;
    float I = ( pi * powf( dia, 4 ) ) / 64.f;  // inertia
    float T = 4.f * _fmin * _fmin * _rho;
    
   // printf("---------------------------------------\n"); 
    for (int n=0; n<_nmodes_req; ++n) {       
        float k = (n+1) * pi;
        const float vf = sc_clip(_fc[n] * ( 1 + _detune ), 20.f, flim );
        const float omega = vf * twopi;
        const float d = _d1 + _d2 * (vf * vf * sr1_squared);
       ...
       // printf( "mode %d \t %f \t %f \t %f \t %f \t %f \n", n, vf, _Kbridge[n], vy1, vy2, vy3 );
    }
}

void PHMResonFriction::UpdateFingerCoeffs(){
    _A01=0.f;
    _A11=0.f;
    _B00=0.f;
    _B01=0.f;
    //printf("---------------------------------------\n"); 
    float vbowpos = _bow_pos;
    float vfingerpos = _finger_pos;
    float s2L = sqrtf(2.f);
...
PluginLoad(PHMResonFrictionUGens) {
    // Plugin magic
    ft = inTable;
    registerUnit<PHMResonFriction::PHMResonFriction>(ft, "PHMResonFriction", false);
}
